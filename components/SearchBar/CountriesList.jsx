import React from 'react'

function CountriesList() {
  return (
    <datalist id='countries'>
      <option value='Nuuk'></option>
      <option value='Greenland'></option>
      <option value='Greenland'></option>
      <option value='San Salvador'></option>
      <option value='El Salvador'></option>
      <option value='Republic of El Salvador'></option>
      <option value='Tbilisi'></option>
      <option value='Georgia'></option>
      <option value='Georgia'></option>
      <option value='Georgetown'></option>
      <option value='Guyana'></option>
      <option value='Co-operative Republic of Guyana'></option>
      <option value='Podgorica'></option>
      <option value='Montenegro'></option>
      <option value='Montenegro'></option>
      <option value='Athens'></option>
      <option value='Greece'></option>
      <option value='Hellenic Republic'></option>
      <option value='Gaborone'></option>
      <option value='Botswana'></option>
      <option value='Republic of Botswana'></option>
      <option value='Sri Jayawardenepura Kotte'></option>
      <option value='Sri Lanka'></option>
      <option value='Democratic Socialist Republic of Sri Lanka'></option>
      <option value='Muscat'></option>
      <option value='Oman'></option>
      <option value='Sultanate of Oman'></option>
      <option value='Vilnius'></option>
      <option value='Lithuania'></option>
      <option value='Republic of Lithuania'></option>
      <option value='Malé'></option>
      <option value='Maldives'></option>
      <option value='Republic of the Maldives'></option>
      <option value='Lisbon'></option>
      <option value='Portugal'></option>
      <option value='Portuguese Republic'></option>
      <option value='Apia'></option>
      <option value='Samoa'></option>
      <option value='Independent State of Samoa'></option>
      <option value='Freetown'></option>
      <option value='Sierra Leone'></option>
      <option value='Republic of Sierra Leone'></option>
      <option value='Bandar Seri Begawan'></option>
      <option value='Brunei'></option>
      <option value='Nation of Brunei, Abode of Peace'></option>
      <option value='Road Town'></option>
      <option value='British Virgin Islands'></option>
      <option value='Virgin Islands'></option>
      <option value='Kingston'></option>
      <option value='Jamaica'></option>
      <option value='Jamaica'></option>
      <option value='Saipan'></option>
      <option value='Northern Mariana Islands'></option>
      <option value='Commonwealth of the Northern Mariana Islands'></option>
      <option value='City of San Marino'></option>
      <option value='San Marino'></option>
      <option value='Republic of San Marino'></option>
      <option value='Lilongwe'></option>
      <option value='Malawi'></option>
      <option value='Republic of Malawi'></option>
      <option value='Kuala Lumpur'></option>
      <option value='Malaysia'></option>
      <option value='Malaysia'></option>
      <option value='Pago Pago'></option>
      <option value='American Samoa'></option>
      <option value='American Samoa'></option>
      <option value="N'Djamena"></option>
      <option value='Chad'></option>
      <option value='Republic of Chad'></option>
      <option value='Taipei'></option>
      <option value='Taiwan'></option>
      <option value='Republic of China (Taiwan)'></option>
      <option value='The Valley'></option>
      <option value='Anguilla'></option>
      <option value='Anguilla'></option>
      <option value="Saint John's"></option>
      <option value='Antigua and Barbuda'></option>
      <option value='Antigua and Barbuda'></option>
      <option value='Djibouti'></option>
      <option value='Djibouti'></option>
      <option value='Republic of Djibouti'></option>
      <option value='Flying Fish Cove'></option>
      <option value='Christmas Island'></option>
      <option value='Territory of Christmas Island'></option>
      <option value='Dublin'></option>
      <option value='Ireland'></option>
      <option value='Republic of Ireland'></option>
      <option value='Havana'></option>
      <option value='Cuba'></option>
      <option value='Republic of Cuba'></option>
      <option value='Port Louis'></option>
      <option value='Mauritius'></option>
      <option value='Republic of Mauritius'></option>
      <option value='Sofia'></option>
      <option value='Bulgaria'></option>
      <option value='Republic of Bulgaria'></option>
      <option value='Basse-Terre'></option>
      <option value='Guadeloupe'></option>
      <option value='Guadeloupe'></option>
      <option value='Bissau'></option>
      <option value='Guinea-Bissau'></option>
      <option value='Republic of Guinea-Bissau'></option>
      <option value='Cairo'></option>
      <option value='Egypt'></option>
      <option value='Arab Republic of Egypt'></option>
      <option value='Fort-de-France'></option>
      <option value='Martinique'></option>
      <option value='Martinique'></option>
      <option value='Nassau'></option>
      <option value='Bahamas'></option>
      <option value='Commonwealth of the Bahamas'></option>
      <option value='Berlin'></option>
      <option value='Germany'></option>
      <option value='Federal Republic of Germany'></option>
      <option value='Tripoli'></option>
      <option value='Libya'></option>
      <option value='State of Libya'></option>
      <option value='Buenos Aires'></option>
      <option value='Argentina'></option>
      <option value='Argentine Republic'></option>
      <option value='Caracas'></option>
      <option value='Venezuela'></option>
      <option value='Bolivarian Republic of Venezuela'></option>
      <option value='Juba'></option>
      <option value='South Sudan'></option>
      <option value='Republic of South Sudan'></option>
      <option value='Moroni'></option>
      <option value='Comoros'></option>
      <option value='Union of the Comoros'></option>
      <option value='Sarajevo'></option>
      <option value='Bosnia and Herzegovina'></option>
      <option value='Bosnia and Herzegovina'></option>
      <option value='Niamey'></option>
      <option value='Niger'></option>
      <option value='Republic of Niger'></option>
      <option value='Managua'></option>
      <option value='Nicaragua'></option>
      <option value='Republic of Nicaragua'></option>
      <option value='Lusaka'></option>
      <option value='Zambia'></option>
      <option value='Republic of Zambia'></option>
      <option value='Gustavia'></option>
      <option value='Saint Barthélemy'></option>
      <option value='Collectivity of Saint Barthélemy'></option>
      <option value='Roseau'></option>
      <option value='Dominica'></option>
      <option value='Commonwealth of Dominica'></option>
      <option value='Conakry'></option>
      <option value='Guinea'></option>
      <option value='Republic of Guinea'></option>
      <option value='Kinshasa'></option>
      <option value='DR Congo'></option>
      <option value='Democratic Republic of the Congo'></option>
      <option value='Cayenne'></option>
      <option value='French Guiana'></option>
      <option value='Guiana'></option>
      <option value='Riyadh'></option>
      <option value='Saudi Arabia'></option>
      <option value='Kingdom of Saudi Arabia'></option>
      <option value='Bangui'></option>
      <option value='Central African Republic'></option>
      <option value='Central African Republic'></option>
      <option value='Andorra la Vella'></option>
      <option value='Andorra'></option>
      <option value='Principality of Andorra'></option>
      <option value='Kingstown'></option>
      <option value='Saint Vincent and the Grenadines'></option>
      <option value='Saint Vincent and the Grenadines'></option>
      <option value='Tallinn'></option>
      <option value='Estonia'></option>
      <option value='Republic of Estonia'></option>
      <option value='Tórshavn'></option>
      <option value='Faroe Islands'></option>
      <option value='Faroe Islands'></option>
      <option value='Vatican City'></option>
      <option value='Vatican City'></option>
      <option value='Vatican City State'></option>
      <option value='Vienna'></option>
      <option value='Austria'></option>
      <option value='Republic of Austria'></option>
      <option value='Bogotá'></option>
      <option value='Colombia'></option>
      <option value='Republic of Colombia'></option>
      <option value='Dakar'></option>
      <option value='Senegal'></option>
      <option value='Republic of Senegal'></option>
      <option value='Singapore'></option>
      <option value='Singapore'></option>
      <option value='Republic of Singapore'></option>
      <option value='Vientiane'></option>
      <option value='Laos'></option>
      <option value="Lao People's Democratic Republic"></option>
      <option value='George Town'></option>
      <option value='Cayman Islands'></option>
      <option value='Cayman Islands'></option>
      <option value='Mbabane'></option>
      <option value='Eswatini'></option>
      <option value='Kingdom of Eswatini'></option>
      <option value='Khartoum'></option>
      <option value='Sudan'></option>
      <option value='Republic of the Sudan'></option>
      <option value='Funafuti'></option>
      <option value='Tuvalu'></option>
      <option value='Tuvalu'></option>
      <option value='Damascus'></option>
      <option value='Syria'></option>
      <option value='Syrian Arab Republic'></option>
      <option value='Washington DC'></option>
      <option value='United States Minor Outlying Islands'></option>
      <option value='United States Minor Outlying Islands'></option>
      <option value='Monrovia'></option>
      <option value='Liberia'></option>
      <option value='Republic of Liberia'></option>
      <option value='Majuro'></option>
      <option value='Marshall Islands'></option>
      <option value='Republic of the Marshall Islands'></option>
      <option value='Bern'></option>
      <option value='Switzerland'></option>
      <option value='Swiss Confederation'></option>
      <option value='Reykjavik'></option>
      <option value='Iceland'></option>
      <option value='Iceland'></option>
      <option value='Cockburn Town'></option>
      <option value='Turks and Caicos Islands'></option>
      <option value='Turks and Caicos Islands'></option>
      <option value='Harare'></option>
      <option value='Zimbabwe'></option>
      <option value='Republic of Zimbabwe'></option>
      <option value='Brasília'></option>
      <option value='Brazil'></option>
      <option value='Federative Republic of Brazil'></option>
      <option value='Kigali'></option>
      <option value='Rwanda'></option>
      <option value='Republic of Rwanda'></option>
      <option value='Vaduz'></option>
      <option value='Liechtenstein'></option>
      <option value='Principality of Liechtenstein'></option>
      <option value='Tashkent'></option>
      <option value='Uzbekistan'></option>
      <option value='Republic of Uzbekistan'></option>
      <option value='Mata-Utu'></option>
      <option value='Wallis and Futuna'></option>
      <option value='Territory of the Wallis and Futuna Islands'></option>
      <option value='Bratislava'></option>
      <option value='Slovakia'></option>
      <option value='Slovak Republic'></option>
      <option value='Plymouth'></option>
      <option value='Montserrat'></option>
      <option value='Montserrat'></option>
      <option value='Washington, D.C.'></option>
      <option value='United States'></option>
      <option value='United States of America'></option>
      <option value='Dhaka'></option>
      <option value='Bangladesh'></option>
      <option value="People's Republic of Bangladesh"></option>
      <option value='Gibraltar'></option>
      <option value='Gibraltar'></option>
      <option value='Gibraltar'></option>
      <option value='Oranjestad'></option>
      <option value='Aruba'></option>
      <option value='Aruba'></option>
      <option value='Panama City'></option>
      <option value='Panama'></option>
      <option value='Republic of Panama'></option>
      <option value='Manila'></option>
      <option value='Philippines'></option>
      <option value='Republic of the Philippines'></option>
      <option value='Quito'></option>
      <option value='Ecuador'></option>
      <option value='Republic of Ecuador'></option>
      <option value='Moscow'></option>
      <option value='Russia'></option>
      <option value='Russian Federation'></option>
      <option value='Maputo'></option>
      <option value='Mozambique'></option>
      <option value='Republic of Mozambique'></option>
      <option value='Jamestown'></option>
      <option value='Saint Helena, Ascension and Tristan da Cunha'></option>
      <option value='Saint Helena, Ascension and Tristan da Cunha'></option>
      <option value='Tehran'></option>
      <option value='Iran'></option>
      <option value='Islamic Republic of Iran'></option>
      <option value='Ottawa'></option>
      <option value='Canada'></option>
      <option value='Canada'></option>
      <option value='Kabul'></option>
      <option value='Afghanistan'></option>
      <option value='Islamic Republic of Afghanistan'></option>
      <option value='Madrid'></option>
      <option value='Spain'></option>
      <option value='Kingdom of Spain'></option>
      <option value="St. George's"></option>
      <option value='Grenada'></option>
      <option value='Grenada'></option>
      <option value='Mamoudzou'></option>
      <option value='Mayotte'></option>
      <option value='Department of Mayotte'></option>
      <option value='Stanley'></option>
      <option value='Falkland Islands'></option>
      <option value='Falkland Islands'></option>
      <option value='Bucharest'></option>
      <option value='Romania'></option>
      <option value='Romania'></option>
      <option value='Baku'></option>
      <option value='Azerbaijan'></option>
      <option value='Republic of Azerbaijan'></option>
      <option value='St. Peter Port'></option>
      <option value='Guernsey'></option>
      <option value='Bailiwick of Guernsey'></option>
      <option value='Phnom Penh'></option>
      <option value='Cambodia'></option>
      <option value='Kingdom of Cambodia'></option>
      <option value='Paramaribo'></option>
      <option value='Suriname'></option>
      <option value='Republic of Suriname'></option>
      <option value='undefined'></option>
      <option value='Bouvet Island'></option>
      <option value='Bouvet Island'></option>
      <option value='Algiers'></option>
      <option value='Algeria'></option>
      <option value="People's Democratic Republic of Algeria"></option>
      <option value='Yerevan'></option>
      <option value='Armenia'></option>
      <option value='Republic of Armenia'></option>
      <option value='Port-aux-Français'></option>
      <option value='French Southern and Antarctic Lands'></option>
      <option value='Territory of the French Southern and Antarctic Lands'></option>
      <option value='Rabat'></option>
      <option value='Morocco'></option>
      <option value='Kingdom of Morocco'></option>
      <option value='Bridgetown'></option>
      <option value='Barbados'></option>
      <option value='Barbados'></option>
      <option value='Luanda'></option>
      <option value='Angola'></option>
      <option value='Republic of Angola'></option>
      <option value='Honiara'></option>
      <option value='Solomon Islands'></option>
      <option value='Solomon Islands'></option>
      <option value='Bamako'></option>
      <option value='Mali'></option>
      <option value='Republic of Mali'></option>
      <option value='Nouméa'></option>
      <option value='New Caledonia'></option>
      <option value='New Caledonia'></option>
      <option value='Brussels'></option>
      <option value='Belgium'></option>
      <option value='Kingdom of Belgium'></option>
      <option value='Suva'></option>
      <option value='Fiji'></option>
      <option value='Republic of Fiji'></option>
      <option value='Riga'></option>
      <option value='Latvia'></option>
      <option value='Republic of Latvia'></option>
      <option value='Mexico City'></option>
      <option value='Mexico'></option>
      <option value='United Mexican States'></option>
      <option value='undefined'></option>
      <option value='Macau'></option>
      <option value="Macao Special Administrative Region of the People's Republic of China"></option>
      <option value='Manama'></option>
      <option value='Bahrain'></option>
      <option value='Kingdom of Bahrain'></option>
      <option value="Sana'a"></option>
      <option value='Yemen'></option>
      <option value='Republic of Yemen'></option>
      <option value='Kralendijk'></option>
      <option value='Caribbean Netherlands'></option>
      <option value='Bonaire, Sint Eustatius and Saba'></option>
      <option value='Charlotte Amalie'></option>
      <option value='United States Virgin Islands'></option>
      <option value='Virgin Islands of the United States'></option>
      <option value='Nouakchott'></option>
      <option value='Mauritania'></option>
      <option value='Islamic Republic of Mauritania'></option>
      <option value='Avarua'></option>
      <option value='Cook Islands'></option>
      <option value='Cook Islands'></option>
      <option value='Monaco'></option>
      <option value='Monaco'></option>
      <option value='Principality of Monaco'></option>
      <option value='Alofi'></option>
      <option value='Niue'></option>
      <option value='Niue'></option>
      <option value='Lima'></option>
      <option value='Peru'></option>
      <option value='Republic of Peru'></option>
      <option value='Diego Garcia'></option>
      <option value='British Indian Ocean Territory'></option>
      <option value='British Indian Ocean Territory'></option>
      <option value='Belgrade'></option>
      <option value='Serbia'></option>
      <option value='Republic of Serbia'></option>
      <option value='Dodoma'></option>
      <option value='Tanzania'></option>
      <option value='United Republic of Tanzania'></option>
      <option value='Islamabad'></option>
      <option value='Pakistan'></option>
      <option value='Islamic Republic of Pakistan'></option>
      <option value='Minsk'></option>
      <option value='Belarus'></option>
      <option value='Republic of Belarus'></option>
      <option value='Castries'></option>
      <option value='Saint Lucia'></option>
      <option value='Saint Lucia'></option>
      <option value='San Juan'></option>
      <option value='Puerto Rico'></option>
      <option value='Commonwealth of Puerto Rico'></option>
      <option value='Naypyidaw'></option>
      <option value='Myanmar'></option>
      <option value='Republic of the Union of Myanmar'></option>
      <option value='Victoria'></option>
      <option value='Seychelles'></option>
      <option value='Republic of Seychelles'></option>
      <option value='Hamilton'></option>
      <option value='Bermuda'></option>
      <option value='Bermuda'></option>
      <option value='Kyiv'></option>
      <option value='Ukraine'></option>
      <option value='Ukraine'></option>
      <option value='Banjul'></option>
      <option value='Gambia'></option>
      <option value='Republic of the Gambia'></option>
      <option value='Asunción'></option>
      <option value='Paraguay'></option>
      <option value='Republic of Paraguay'></option>
      <option value='São Tomé'></option>
      <option value='São Tomé and Príncipe'></option>
      <option value='Democratic Republic of São Tomé and Príncipe'></option>
      <option value='Copenhagen'></option>
      <option value='Denmark'></option>
      <option value='Kingdom of Denmark'></option>
      <option value='Seoul'></option>
      <option value='South Korea'></option>
      <option value='Republic of Korea'></option>
      <option value='Saint-Pierre'></option>
      <option value='Saint Pierre and Miquelon'></option>
      <option value='Saint Pierre and Miquelon'></option>
      <option value='Tunis'></option>
      <option value='Tunisia'></option>
      <option value='Tunisian Republic'></option>
      <option value='Kuwait City'></option>
      <option value='Kuwait'></option>
      <option value='State of Kuwait'></option>
      <option value='Rome'></option>
      <option value='Italy'></option>
      <option value='Italian Republic'></option>
      <option value='King Edward Point'></option>
      <option value='South Georgia'></option>
      <option value='South Georgia and the South Sandwich Islands'></option>
      <option value='Mariehamn'></option>
      <option value='Åland Islands'></option>
      <option value='Åland Islands'></option>
      <option value='Nicosia'></option>
      <option value='Cyprus'></option>
      <option value='Republic of Cyprus'></option>
      <option value='Luxembourg'></option>
      <option value='Luxembourg'></option>
      <option value='Grand Duchy of Luxembourg'></option>
      <option value='Port Moresby'></option>
      <option value='Papua New Guinea'></option>
      <option value='Independent State of Papua New Guinea'></option>
      <option value='Lomé'></option>
      <option value='Togo'></option>
      <option value='Togolese Republic'></option>
      <option value='City of Victoria'></option>
      <option value='Hong Kong'></option>
      <option value="Hong Kong Special Administrative Region of the People's Republic of China"></option>
      <option value='South Tarawa'></option>
      <option value='Kiribati'></option>
      <option value='Independent and Sovereign Republic of Kiribati'></option>
      <option value='Antananarivo'></option>
      <option value='Madagascar'></option>
      <option value='Republic of Madagascar'></option>
      <option value='London'></option>
      <option value='United Kingdom'></option>
      <option value='United Kingdom of Great Britain and Northern Ireland'></option>
      <option value='Sucre'></option>
      <option value='Bolivia'></option>
      <option value='Plurinational State of Bolivia'></option>
      <option value='Douglas'></option>
      <option value='Isle of Man'></option>
      <option value='Isle of Man'></option>
      <option value='Ljubljana'></option>
      <option value='Slovenia'></option>
      <option value='Republic of Slovenia'></option>
      <option value='Saint-Denis'></option>
      <option value='Réunion'></option>
      <option value='Réunion Island'></option>
      <option value='Bishkek'></option>
      <option value='Kyrgyzstan'></option>
      <option value='Kyrgyz Republic'></option>
      <option value='Wellington'></option>
      <option value='New Zealand'></option>
      <option value='New Zealand'></option>
      <option value='Nur-Sultan'></option>
      <option value='Kazakhstan'></option>
      <option value='Republic of Kazakhstan'></option>
      <option value='Skopje'></option>
      <option value='North Macedonia'></option>
      <option value='Republic of North Macedonia'></option>
      <option value='undefined'></option>
      <option value='Heard Island and McDonald Islands'></option>
      <option value='Heard Island and McDonald Islands'></option>
      <option value="Nuku'alofa"></option>
      <option value='Tonga'></option>
      <option value='Kingdom of Tonga'></option>
      <option value='Pristina'></option>
      <option value='Kosovo'></option>
      <option value='Republic of Kosovo'></option>
      <option value='Santiago'></option>
      <option value='Chile'></option>
      <option value='Republic of Chile'></option>
      <option value='Gitega'></option>
      <option value='Burundi'></option>
      <option value='Republic of Burundi'></option>
      <option value='Santo Domingo'></option>
      <option value='Dominican Republic'></option>
      <option value='Dominican Republic'></option>
      <option value='El Aaiún'></option>
      <option value='Western Sahara'></option>
      <option value='Sahrawi Arab Democratic Republic'></option>
      <option value='Maseru'></option>
      <option value='Lesotho'></option>
      <option value='Kingdom of Lesotho'></option>
      <option value='Pyongyang'></option>
      <option value='North Korea'></option>
      <option value="Democratic People's Republic of Korea"></option>
      <option value='West Island'></option>
      <option value='Cocos (Keeling) Islands'></option>
      <option value='Territory of the Cocos (Keeling) Islands'></option>
      <option value='Thimphu'></option>
      <option value='Bhutan'></option>
      <option value='Kingdom of Bhutan'></option>
      <option value='Mogadishu'></option>
      <option value='Somalia'></option>
      <option value='Federal Republic of Somalia'></option>
      <option value='Amsterdam'></option>
      <option value='Netherlands'></option>
      <option value='Kingdom of the Netherlands'></option>
      <option value='Dushanbe'></option>
      <option value='Tajikistan'></option>
      <option value='Republic of Tajikistan'></option>
      <option value='Dili'></option>
      <option value='Timor-Leste'></option>
      <option value='Democratic Republic of Timor-Leste'></option>
      <option value='Beirut'></option>
      <option value='Lebanon'></option>
      <option value='Lebanese Republic'></option>
      <option value='Philipsburg'></option>
      <option value='Sint Maarten'></option>
      <option value='Sint Maarten'></option>
      <option value='Kampala'></option>
      <option value='Uganda'></option>
      <option value='Republic of Uganda'></option>
      <option value='Accra'></option>
      <option value='Ghana'></option>
      <option value='Republic of Ghana'></option>
      <option value='Tirana'></option>
      <option value='Albania'></option>
      <option value='Republic of Albania'></option>
      <option value='Ngerulmud'></option>
      <option value='Palau'></option>
      <option value='Republic of Palau'></option>
      <option value='Palikir'></option>
      <option value='Micronesia'></option>
      <option value='Federated States of Micronesia'></option>
      <option value='Willemstad'></option>
      <option value='Curaçao'></option>
      <option value='Country of Curaçao'></option>
      <option value='Helsinki'></option>
      <option value='Finland'></option>
      <option value='Republic of Finland'></option>
      <option value='Saint Helier'></option>
      <option value='Jersey'></option>
      <option value='Bailiwick of Jersey'></option>
      <option value='Windhoek'></option>
      <option value='Namibia'></option>
      <option value='Republic of Namibia'></option>
      <option value='Hanoi'></option>
      <option value='Vietnam'></option>
      <option value='Socialist Republic of Vietnam'></option>
      <option value='undefined'></option>
      <option value='Antarctica'></option>
      <option value='Antarctica'></option>
      <option value='Ramallah'></option>
      <option value='Palestine'></option>
      <option value='State of Palestine'></option>
      <option value='Ankara'></option>
      <option value='Turkey'></option>
      <option value='Republic of Turkey'></option>
      <option value='Chișinău'></option>
      <option value='Moldova'></option>
      <option value='Republic of Moldova'></option>
      <option value='Basseterre'></option>
      <option value='Saint Kitts and Nevis'></option>
      <option value='Federation of Saint Christopher and Nevis'></option>
      <option value='Canberra'></option>
      <option value='Australia'></option>
      <option value='Commonwealth of Australia'></option>
      <option value='Tokyo'></option>
      <option value='Japan'></option>
      <option value='Japan'></option>
      <option value='Zagreb'></option>
      <option value='Croatia'></option>
      <option value='Republic of Croatia'></option>
      <option value='Ouagadougou'></option>
      <option value='Burkina Faso'></option>
      <option value='Burkina Faso'></option>
      <option value='Jakarta'></option>
      <option value='Indonesia'></option>
      <option value='Republic of Indonesia'></option>
      <option value='Ashgabat'></option>
      <option value='Turkmenistan'></option>
      <option value='Turkmenistan'></option>
      <option value='Brazzaville'></option>
      <option value='Republic of the Congo'></option>
      <option value='Republic of the Congo'></option>
      <option value='Port Vila'></option>
      <option value='Vanuatu'></option>
      <option value='Republic of Vanuatu'></option>
      <option value='Belmopan'></option>
      <option value='Belize'></option>
      <option value='Belize'></option>
      <option value='Addis Ababa'></option>
      <option value='Ethiopia'></option>
      <option value='Federal Democratic Republic of Ethiopia'></option>
      <option value='Tegucigalpa'></option>
      <option value='Honduras'></option>
      <option value='Republic of Honduras'></option>
      <option value='Warsaw'></option>
      <option value='Poland'></option>
      <option value='Republic of Poland'></option>
      <option value='Yaren'></option>
      <option value='Nauru'></option>
      <option value='Republic of Nauru'></option>
      <option value='Kingston'></option>
      <option value='Norfolk Island'></option>
      <option value='Territory of Norfolk Island'></option>
      <option value='Doha'></option>
      <option value='Qatar'></option>
      <option value='State of Qatar'></option>
      <option value='Port-au-Prince'></option>
      <option value='Haiti'></option>
      <option value='Republic of Haiti'></option>
      <option value='Montevideo'></option>
      <option value='Uruguay'></option>
      <option value='Oriental Republic of Uruguay'></option>
      <option value='Nairobi'></option>
      <option value='Kenya'></option>
      <option value='Republic of Kenya'></option>
      <option value='Malabo'></option>
      <option value='Equatorial Guinea'></option>
      <option value='Republic of Equatorial Guinea'></option>
      <option value='Porto-Novo'></option>
      <option value='Benin'></option>
      <option value='Republic of Benin'></option>
      <option value='Paris'></option>
      <option value='France'></option>
      <option value='French Republic'></option>
      <option value='Bangkok'></option>
      <option value='Thailand'></option>
      <option value='Kingdom of Thailand'></option>
      <option value='Hagåtña'></option>
      <option value='Guam'></option>
      <option value='Guam'></option>
      <option value='Marigot'></option>
      <option value='Saint Martin'></option>
      <option value='Saint Martin'></option>
      <option value='New Delhi'></option>
      <option value='India'></option>
      <option value='Republic of India'></option>
      <option value='Amman'></option>
      <option value='Jordan'></option>
      <option value='Hashemite Kingdom of Jordan'></option>
      <option value='San José'></option>
      <option value='Costa Rica'></option>
      <option value='Republic of Costa Rica'></option>
      <option value='Kathmandu'></option>
      <option value='Nepal'></option>
      <option value='Federal Democratic Republic of Nepal'></option>
      <option value='Praia'></option>
      <option value='Cape Verde'></option>
      <option value='Republic of Cabo Verde'></option>
      <option value='Port of Spain'></option>
      <option value='Trinidad and Tobago'></option>
      <option value='Republic of Trinidad and Tobago'></option>
      <option value='Pretoria,Bloemfontein,Cape Town'></option>
      <option value='South Africa'></option>
      <option value='Republic of South Africa'></option>
      <option value='Libreville'></option>
      <option value='Gabon'></option>
      <option value='Gabonese Republic'></option>
      <option value='Asmara'></option>
      <option value='Eritrea'></option>
      <option value='State of Eritrea'></option>
      <option value='Jerusalem'></option>
      <option value='Israel'></option>
      <option value='State of Israel'></option>
      <option value='Abuja'></option>
      <option value='Nigeria'></option>
      <option value='Federal Republic of Nigeria'></option>
      <option value='Oslo'></option>
      <option value='Norway'></option>
      <option value='Kingdom of Norway'></option>
      <option value='Papeetē'></option>
      <option value='French Polynesia'></option>
      <option value='French Polynesia'></option>
      <option value='Stockholm'></option>
      <option value='Sweden'></option>
      <option value='Kingdom of Sweden'></option>
      <option value='Baghdad'></option>
      <option value='Iraq'></option>
      <option value='Republic of Iraq'></option>
      <option value='Adamstown'></option>
      <option value='Pitcairn Islands'></option>
      <option value='Pitcairn Group of Islands'></option>
      <option value='Fakaofo'></option>
      <option value='Tokelau'></option>
      <option value='Tokelau'></option>
      <option value='Yaoundé'></option>
      <option value='Cameroon'></option>
      <option value='Republic of Cameroon'></option>
      <option value='Beijing'></option>
      <option value='China'></option>
      <option value="People's Republic of China"></option>
      <option value='Abu Dhabi'></option>
      <option value='United Arab Emirates'></option>
      <option value='United Arab Emirates'></option>
      <option value='Yamoussoukro'></option>
      <option value='Ivory Coast'></option>
      <option value="Republic of Côte d'Ivoire"></option>
      <option value='Valletta'></option>
      <option value='Malta'></option>
      <option value='Republic of Malta'></option>
      <option value='Ulan Bator'></option>
      <option value='Mongolia'></option>
      <option value='Mongolia'></option>
      <option value='Guatemala City'></option>
      <option value='Guatemala'></option>
      <option value='Republic of Guatemala'></option>
      <option value='Prague'></option>
      <option value='Czechia'></option>
      <option value='Czech Republic'></option>
      <option value='Budapest'></option>
      <option value='Hungary'></option>
      <option value='Hungary'></option>
      <option value='Longyearbyen'></option>
      <option value='Svalbard and Jan Mayen'></option>
      <option value='Svalbard og Jan Mayen'></option>
    </datalist>
  )
}

export default CountriesList
